...
**MUST**: 所有远程操作通过 dispatch 层执行，使用 `python ~/.workbuddy/skills/ssh-skill/scripts/脚本名.py` 格式。使用别名（alias）标识服务器。

**NEVER**: 不要绕过 dispatch 直接写 `ssh` 或 `scp` 命令，不要使用反斜杠 `\`。
...
**重要**: 文件传输命令**必须**加 `MSYS_NO_PATHCONV=1` 前缀，防止 Windows MSYS bash 将远程路径如 `/tmp/file` 转换为 Windows 路径。上传/下载/服务器间传输均适用。
